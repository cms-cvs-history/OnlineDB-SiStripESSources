...
// -----------------------------------------------------------------------------
// 
test_FedCablingBuilderFromDb::test_FedCablingBuilderFromDb( const edm::ParameterSet& pset ) 
  : db_(0),
    source_( pset.getUntrackedParameter<string>( "CablingSource", "CONNECTIONS" ) )
{
  LogDebug(mlCabling_)
    << "[test_FedCablingBuilderFromDb::" << __func__ << "]"
    << " Constructing object...";
...
// -----------------------------------------------------------------------------
// 
test_FedCablingBuilderFromDb::~test_FedCablingBuilderFromDb() {
  if ( db_ ) { 
...
// -----------------------------------------------------------------------------
// 
void test_FedCablingBuilderFromDb::beginJob( const edm::EventSetup& setup ) {
  ...
  stringstream ss;
  ss << "[test_FedCablingBuilderFromDb::" << __func__ << "]"
     << " Built SiStripFecCabling object with following devices:" << endl
     << endl << devs;
  LogDebug(mlCabling_) << ss.str();

  stringstream sss;
  sss << "[test_FedCablingBuilderFromDb::" << __func__ << "]" << endl;
  sss << fed_cabling;
